"""
验证看板读模型（票 31；票 34 重订边界）：前瞻成绩、mode 隔离与看板数据。

HTTP 路由（api/validation.py）只做参数与视图映射；本模块是验证边界规则
的唯一住所。已结注的读取与决策身份去重走 betting 包的共享口径
（settled_purchased_bets / dedupe_by_decision），与 CLV 报表同源。
"""
...
from goalx_backend.betting.store import (
    count_unpurchased_open,
    dedupe_by_decision,
    settled_purchased_bets,
)
...
def list_backtest_runs(conn: sqlite3.Connection) -> list[BacktestRunView]:
    """全部回测 run（新→旧），附 overall 指标摘要。"""
    rows = conn.execute(
        "SELECT * FROM backtest_runs ORDER BY created_at DESC, id DESC"
    ).fetchall()
    return [_run_view(row, _overall_metrics(conn, int(row["id"]))) for row in rows]


def backtest_run_details(
    conn: sqlite3.Connection, run_id: int
) -> BacktestRunDetailView | None:
...
def _settled_bets(conn: sqlite3.Connection, mode: str) -> tuple[list[Any], int]:
    """某 mode 的已结算已购固定注（决策身份去重后）+ 重复数。"""
    bets = [
        bet
        for bet in settled_purchased_bets(conn)
        if bet.market_kind == "fixed" and bet.mode == mode
    ]
    return dedupe_by_decision(bets)


def _mode_counts(bets: list[Any], duplicates: int) -> ModeCounts:
    ...
    for bet in bets:
        for leg in bet.legs:
            legs += 1
            fixtures.add(leg.fixture_id)
    return ModeCounts(
        bets=len(bets) + duplicates,
        unique_bets=len(bets),
        legs=legs,
        fixtures=len(fixtures),
        staked=round(sum(bet.stake for bet in bets), 2),
        profit=round(sum(bet.profit or 0.0 for bet in bets), 2),
    )


def _yield_curve(bets: list[Any]) -> list[YieldPoint]:
    """唯一注的累计和滚动 yield 曲线（票 34：滚动窗口用唯一纸面 Bet）。"""
...
def _season_condition(bets: list[Any]) -> ConditionProgress:
    """整赛季条件：独立显示；缺真实整赛季证据前恒未完成（票 34 验收 2）。"""
    kickoffs = [str(bet.placed_at or bet.created_at or "")[:10] for bet in bets]
    span = f"{min(kickoffs)}~{max(kickoffs)}" if kickoffs else "无样本"
    return ConditionProgress(
        key="full_season",
        label="整赛季纸面样本覆盖",
        achieved=False,
        current=f"{len(bets)} 唯一注 / {span}(覆盖未验收)",
...
def validation_progress(
    ...
    conditions.append(_season_condition(paper_rows))
    ...
    latest_view: BacktestRunView | None = (
        _run_view(run_row, _overall_metrics(conn, int(run_row["id"])))
        if run_row is not None
        else None
    )
    return ValidationProgressView(
        conditions=conditions,
        paper=_mode_counts(paper_rows, paper_dups),
        live=_mode_counts(live_rows, live_dups),
