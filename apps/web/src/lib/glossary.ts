...
export const GLOSSARY: GlossaryEntry[] = [
	...
	},
	{
		id: "bet-ev-snapshot",
		term: "注级 EV 快照",
		aliases: ["EV 快照", "注级 EV", "平均 EV", "snap_ev_consensus", "snap_ev_model", "双口径 EV"],
		definition:
			"建注时随注落库的锁定时刻 EV（票 41，双存）：共识口径 = 锁定时欧共识 Shin 概率 × 竞彩价 − 1；模型口径 = 锁定时已发出的最新赛前 Forecast（DC）概率 × 竞彩价 − 1；串关按联合概率连乘（两腿独立已声明）与联合锁定赔率。",
		direction:
			"正 = 正期望（红）、负 = 负期望（绿）；历史页平均 EV 共识口径优先、两口径不混算，样本数（有快照注/筛选注数）随指标注明。",
		example:
			"锁定时共识主胜 45%、竞彩价 2.30 → 注级共识 EV = +3.5%；模型主胜 50% → 模型 EV = +15%（双存，复盘两用）；两注平均共识 EV = (+3.5% + 1.5%)/2 = +2.5%。",
		caution:
			"快照是锁定时点的诊断量，不是事后结果；共识/模型两口径分开读、不混算（共识口径优先展示）；口径不可得存 null、存量注无快照（历史页显示“—”）、非 had 腿在 v1 可映射口径之外（与 CLV 同界），均值分母只含有快照的注。",
	},
	{
		id: "pool-ev",
		term: "彩池 EV",
		aliases: ["parimutuel EV", "估计派彩赔率", "公众份额", "抽水折算", "彩池口径 EV"],
		definition:
			"彩池（parimutuel，14场/任9）下一向的期望收益率（票 43）：EV = 概率 × 估计派彩赔率 − 1；估计派彩赔率 = 返奖率 65% ÷ 公众份额（抽水折算，返奖率为规则口径 64% 当期 + 1% 调节基金）。概率优先模型口径（映射场次有赛前 Forecast 时）、退化期次页三向欧指去水；份额为第三方人气分布（公众分布代理，非官方池份额）。",
		direction:
			"正 = 该向相对公众分布被低估（份额低于真实概率应占），负 = 被高估；~35% 结构性抽水下常态轮次多数向为负，正 EV 集中在低份额冷门向。",
		example:
			"概率 32%、公众份额 18% → 估计派彩赔率 = 0.65/0.18 ≈ 3.61，EV ≈ +15.5%；同场热门向概率 45%、份额 50% → 赔率 1.30，EV ≈ −41.5%。",
		caution:
			"未建模 price impact（自己注额推动派彩——个人量级可忽略，Isaacs 1953）与分彩风险（中奖注数多时派彩被摊薄，“人人猜中的热门组合”派彩最低；保守做法用派彩分位数而非均值，v1 用均值口径）；份额是第三方人气代理而非官方池份额，抽样偏差会传导进 EV；常态轮次期望为负是诚实预期（研究 03 §4.3）。",
	},
	{
		id: "pool-cold-value",
		term: "搏冷价值",
		aliases: ["冷门正期望", "值得搏冷", "冷选项", "cold value"],
		definition:
			"一场中冷门向是否值得搏的判定（票 pool-v2/01）：冷选项 = 公众份额 <25% 且彩池 EV 为正；搏冷标记 = 冷选项中 EV 最高、且 EV 高于本场推荐（最高概率）向——即牺牲命中率换赔率是正期望且优于稳妥选。判定完全由既有 EV 公式导出，不另训模型。",
		direction:
			"有搏冷标记 = 该场存在正期望冷门（模型概率显著高于公众份额）；无标记 = 冷门 EV 均为负或不如推荐向，跟大众是更优解。标记处悬浮可见判定理由（概率 vs 份额 vs 估计赔率 vs EV）。",
		example:
			"客胜概率 34%、公众份额 18% → 估计赔率 3.61、EV ≈ +23%；推荐位主胜概率 42%、EV −20% → 搏冷标记落客胜（用 8 个点的命中率换约 43 个点的 EV 差）。",
		caution:
			"搏冷单场命中率天然低（本例 34% vs 42%），价值在长期期望而非单期命中；EV 均值口径未建模分彩摊薄（冷门组合中奖注少、摊薄风险低，是其结构优势）；阈值 25% 是口径初值，实证偏差可调。",
	},
];
